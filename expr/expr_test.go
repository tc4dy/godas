...
func TestAnd(t *testing.T) {
	e := expr.And(expr.Col("age").Gt(20.0), expr.Col("city").Eq("NY"))
...
func TestOr(t *testing.T) {
	e := expr.Or(expr.Col("age").Lt(20.0), expr.Col("city").Eq("SF"))
	mask, err := e.Eval(makeColMap(), 4)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNot(t *testing.T) {
	e := expr.Not(expr.Col("age").Gt(30.0))
	mask, err := e.Eval(makeColMap(), 4)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMissingColumn(t *testing.T) {
	_, err := expr.Col("nonexistent").Gt(0.0).Eval(makeColMap(), 4)
	if err == nil {
